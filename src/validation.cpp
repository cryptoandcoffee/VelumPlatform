...
#if defined(NDEBUG)
# error "Velum Core cannot be compiled without assertions."
#endif
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s VLM additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (!nPrevHeight && Params().NetworkIDString() != "test" || nPrevHeight <= 1) { // No Premine on testnet
        return 35000 * COIN;
    }
    ...
    if (nPrevHeight < 1940) {
	 ...
	 if (nPrevHeight >= 1940 && nPrevHeight < 2660) {
		return 2 * COIN; 
	}
    if (nPrevHeight >= 2660 && nPrevHeight < 3560) {
		return 8 * COIN; 
	}
	if (nPrevHeight >= 3560 && nPrevHeight < 4000) {
        return 10 * COIN; 
	}
    // Total supply 11,000,000
    if (nPrevHeight >= 687500) {
        return 0;
    }
    ...
    // Block reward starts at 16 and declines 50% every 2 years, getting in 10 years ~11M VLM.
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // VELUM : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(VELUM): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(VELUM): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // VELUM : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(VELUM): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(VELUM): spork is off, skipping transaction locking checks\n");
    }
