...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"velumprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"velumprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(const CVelumAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CVelumAddress(CScriptID(script)), strLabel);
    }
}
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Velum address or script");
    }
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CVelumAddress(pubKey.GetID()), strLabel);
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"velumaddress\"\n"
            "\nReveals the private key corresponding to 'velumaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"velumaddress\"   (string, required) The velum address for the private key\n"
            "\nResult:\n"
    ...
    return CVelumSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Velum Core %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CVelumAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CVelumSecret(key).ToString(), strTime);
