...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in velum, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in Velum
//
...
// ***TODO*** that part changed in velum, we are using a mix with old one here for now
void static VelumMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("VelumMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("velum-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("VelumMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("VelumMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("VelumMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("VelumMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateVelums(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VelumMiner, boost::cref(chainparams), boost::ref(connman)));
}
