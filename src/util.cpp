...
const char * const VELUM_CONF_FILENAME = "velum.conf";
const char * const VELUM_PID_FILENAME = "velumd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "velum" is a composite category enabling all Velum-related debug output
            if(ptrCategory->count(string("velum"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VelumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VelumCore
    // Mac: ~/Library/Application Support/VelumCore
    // Unix: ~/.velumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VelumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VelumCore";
#else
    // Unix
    return pathRet / ".velumcore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", VELUM_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty velum.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override velum.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", VELUM_PID_FILENAME));
