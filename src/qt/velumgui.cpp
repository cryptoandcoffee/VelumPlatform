...
const std::string VelumGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString VelumGUI::DEFAULT_WALLET = "~Default";

VelumGUI::VelumGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
{
    QFontDatabase::addApplicationFont(":/fonts/AUdimat-Regular.otf");
    /* Open CSS when configured */
    this->setStyleSheet(GUIUtil::loadStyleSheet());

    GUIUtil::restoreWindowGeometry("nWindow", QSize(850, 550), this);

    QString windowTitle = tr("Velum") + " - ";
#ifdef ENABLE_WALLET
    ...
    QPixmap pixmapLogo(":/images/light/velum_logo_horizontal");
    ...
    QFrame *frameLeft = new QFrame;
    frameLeft->setFixedWidth(300);
    frameLeft->setFixedHeight(55);
    frameLeft->setObjectName(QStringLiteral("frameStatusbarLeft"));
    QVBoxLayout *layoutLeft = new QVBoxLayout;
    frameLeft->setLayout(layoutLeft);
    layoutLeft->addWidget(progressBarLabel);
    layoutLeft->addWidget(progressBar);
    layoutLeft->addWidget(labelBlocksIcon);
    labelLogo->setPixmap(pixmapLogo);
    labelLogo->setAlignment(Qt::AlignCenter);
    layoutLogo->addWidget(frameLeft);
    layoutLogo->addWidget(labelLogo);
    frameBlocks->setObjectName(QStringLiteral("frameStatusbarRight"));
    layoutLogo->addWidget(frameBlocks);
    frameBlocks->setFixedWidth(300);
    statusbar->setLayout(layoutLogo);

    // Create the toolbars
    createToolBars(statusbar);
...
void VelumGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Velum address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and velum: URIs)"));
    ...
    historyAction = new QAction(QIcon(":/icons/" + theme + "/history"), tr("&History"), this);
    ...
    settingsAction = new QAction(QIcon(":/icons/" + theme + "/options"), "Settings", this);
    settingsAction->setStatusTip(tr("Wallet Settings"));
    settingsAction->setToolTip(settingsAction->statusTip());
    settingsAction->setCheckable(true);
#ifdef Q_OS_MAC
    if (settings.value("fShowMasternodesTab").toBool()) {
        settingsAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_6));
    } else {
        settingsAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_5));
    }
#else
    if (settings.value("fShowMasternodesTab").toBool()) {
    ...
    aboutAction = new QAction(tr("&About Velum"), this);
    aboutAction->setStatusTip(tr("Show information about Velum"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Velum"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Velum addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Velum addresses"));
    ...
    openAction->setStatusTip(tr("Open a velum: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Velum help message to get a list with possible Velum command-line options"));
    ...
    connect(openInfoAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(openInfoAction, SIGNAL(triggered()), this, SLOT(showInfo()));
    connect(openRPCConsoleAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(openRPCConsoleAction, SIGNAL(triggered()), this, SLOT(showConsole()));
    connect(openGraphAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(openGraphAction, SIGNAL(triggered()), this, SLOT(showGraph()));
    connect(openPeersAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(openPeersAction, SIGNAL(triggered()), this, SLOT(showPeers()));
    connect(openRepairAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
...
void VelumGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
        ...
        */
        ...
        layout->setAlignment(toolbar, Qt::AlignCenter);
        layout->setSpacing(0);
        layout->setContentsMargins(QMargins());
        QWidget *containerWidget = new QWidget();
        containerWidget->setObjectName("test");
        containerWidget->setLayout(layout);
        setCentralWidget(containerWidget);
    }
}

void VelumGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool VelumGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool VelumGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void VelumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void VelumGUI::setWalletActionsEnabled(bool enabled)
{
...
void VelumGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Velum client") + " " + networkStyle->getTitleAddText();
...
void VelumGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void VelumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void VelumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void VelumGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void VelumGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void VelumGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void VelumGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void VelumGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void VelumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void VelumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void VelumGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void VelumGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Velum network", "", count));
    } else {
...
void VelumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void VelumGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void VelumGUI::updateHeadersSyncProgressLabel()
{
...
void VelumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void VelumGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void VelumGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Velum"); // default title
...
void VelumGUI::changeEvent(QEvent *e)
{
...
void VelumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void VelumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void VelumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(VelumUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void VelumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void VelumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool VelumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool VelumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void VelumGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void VelumGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void VelumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void VelumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void VelumGUI::detectShutdown()
{
...
void VelumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void VelumGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void VelumGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(VelumGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void VelumGUI::subscribeToCoreSignals()
{
...
void VelumGUI::unsubscribeFromCoreSignals()
{
...
void VelumGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void VelumGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<VelumUnits::Unit> units = VelumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const VelumUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(VelumUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(VelumUnits::Unit u, VelumUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(VelumUnits::name(u)), this);
...
{
    setText(VelumUnits::name(newUnits));
}
