    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("Velum Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About Velum Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  velum-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_VELUM_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", VelumGUI::DEFAULT_UIPLATFORM));
        }
...
All the Velum in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 VELUM, 0.1 VELUM, 1 VELUM and 10 VELUM -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("Velum Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(VelumGUI *window)
{
    if (!window)
        return nullptr;
