...
{
    CVelumAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered velum address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CVelumAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        if(recipients[0].fUseInstantSend && total > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN){
            Q_EMIT message(tr("Send Coins"), tr("Instant Send doesn't support sending values that high yet. Transactions are currently limited to %1 VLM.").arg(sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)),
        ...
        if(recipients[0].fUseInstantSend) {
            ...
            if(newTx->vin.size() > CTxLockRequest::WARN_MANY_INPUTS) {
                Q_EMIT message(tr("Send Coins"), tr("Used way too many inputs (>%1) for this Instant Send transaction, fees could be huge.").arg(CTxLockRequest::WARN_MANY_INPUTS),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal velum:URI (velum:XyZ...?message=example)
            {
...
{
    QString strAddress = QString::fromStdString(CVelumAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CVelumAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CVelumAddress(sAddress).Get();
