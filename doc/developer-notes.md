...
Velum Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 18092 for connections and services them.

- VelumMiner : Generates coins (if wallet is enabled).
...
However, in open source software such as Velum Core, where everyone uses
...
pay attention to for reviewers of Velum Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Velum Core uses tinyformat, which is type safe. Leave them out to avoid confusion
