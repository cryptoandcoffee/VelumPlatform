...
*Setup instructions for a Gitian build of Velum Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Velum
...
- [Building Velum Core](#building-velum-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-velum-host/README.md).
...
Clone the git repositories for Velum Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/velumcrypto/velum
```
...
in the Velum Core repository under 'Fetch and build inputs' to install sources which require
...
To build Velum Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Velum Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/velum/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/velumcrypto/velum
...
For example:
```bash
URL=https://github.com/crowning-/velum.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit velum=${COMMIT} --url velum=${URL} ../velum/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit velum=${COMMIT} --url velum=${URL} ../velum/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit velum=${COMMIT} --url velum=${URL} ../velum/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the velum git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../velum/contrib/gitian-descriptors/*|sort|uniq )
...
VLMPATH=/some/root/path/velum.git
SIGPATH=/some/root/path/velum-detached-sigs.git

./bin/gbuild --url velum=${VLMPATH},signature=${SIGPATH} ../velum/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/velum-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/velum-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/velum-osx-build.assert
```
...
[velum/gitian.sigs](https://github.com/velumcrypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
