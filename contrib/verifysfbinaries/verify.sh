...
###   This script attempts to download the signature file SHA256SUMS.asc from velum.org
...
WORKINGDIR="/tmp/velum"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://velum.org/bin/"
VERSIONPREFIX="velum-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'velum-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Velum Core binary release signing key installed?"
   fi
